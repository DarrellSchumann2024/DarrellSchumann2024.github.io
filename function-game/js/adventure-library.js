...
const BLOCK_SIZE = 32; // Adjust this value to change block size

function isPlayerAttacking() {
  // Implement logic to check if player is currently attacking (animation, flag, etc.)
...
function getBlock(x, y) {
  ...
  const allBlocks = gridElement.querySelectorAll('.grid-block');
  const index = y * GRID_WIDTH + x;
  return allBlocks[index] || null;
}

function isCrossable(block) {
  return block && block.classList.contains('grass');
}

function isAttackable(block) {
  return block && block.classList.contains('rock');
}

function attack(block) {
  // Implement logic to change block class from 'rock' to 'gravel' (or similar)
  block.classList.remove('rock');
  block.classList.add('gravel');
}

function getReachablePoint(x, y) {
  const block = getBlock(x, y);
  if (block && isCrossable(block)) {
    return {x, y};
  } else {
    return null;
  }
